    ...
    for data in list_of_dict:
        for key, item in data.items():
            if key not in output:
                raise ValueError(f'key {key} not in output')
            # if mark_flag != 0:
            #     if output[key][mark_flag-1].shape[1] != item.shape[1]:
            #         raise ValueError(f'key {key} have size {output[key][mark_flag-1].shape[1]} in {mark_flag-1} but {item.shape[1]} in {mark_flag}')
            # if output[key][mark_flag-1].shape[0] != item.shape[0]:
            #         raise ValueError(f'key {key} have size {output[key][mark_flag-1].shape[0]} in {mark_flag-1} but {item.shape[0]} in {mark_flag}')
...
class DataProto:
    ...
    def copy(self) -> 'DataProto':
        """Deep copy the DataProto

        Returns:
            DataProto: the copied DataProto
        """
        if self.batch is not None:
    ...
    @staticmethod
    def concat(data: List['DataProto'], auto_pad=False, key_list=None, id_pad=None) -> 'DataProto':
        ...
        if auto_pad:
            if id_pad is None:
                raise ValueError('padding id should not be none')
            if not data:
                return DataProto()
            
            input_key_list = ['input_ids', 'attention_mask', 'position_ids', 'prompts']
            response_key_list = ['responses', 'responses_with_info_mask', 'info_mask']
            other_key_list = ['values', 'old_log_probs', 'ref_log_prob', 'token_level_scores', 
                'token_level_rewards', 'advantages', 'returns']
            
            token_padding_list = ['input_ids','prompts','responses', 'responses_with_info_mask']
            if key_list is not None:            
                input_key_list = list(set(key_list) & set(input_key_list))
                response_key_list = list(set(key_list) & set(response_key_list))
                other_key_list = list((set(key_list) - set(input_key_list))-set(response_key_list))
            max_input_len = 0
            max_response_len = 0
            # 第一遍：找到各组的最大长度
            for dp in data:
                ...
                for key in input_key_list:
                    if key in dp.batch and dp.batch[key] is not None:
                        max_input_len = max(max_input_len, dp.batch[key].shape[1])
                
                # 响应相关keys的最大长度  
                for key in response_key_list:
                    if key in dp.batch and dp.batch[key] is not None:
                        max_response_len = max(max_response_len, dp.batch[key].shape[1])
                
                # 价值估计相关keys的最大长度（通常与response相关）
            ...
            max_input_len = max_input_len +1 if max_input_len % 2 ==1 else max_input_len
            max_response_len = max_response_len +1 if max_response_len % 2 ==1 else max_response_len
            # 确保input相关序列长度足够
            max_input_len = max(max_input_len, max_response_len + 2)  # 保证可以切片
            
            # 第二遍：按组进行padding
            for dp in data:
                # 对输入相关keys使用max_input_len
                for key in input_key_list:
                    if key in token_padding_list:
                        padding_idx = id_pad
                    else:
                        padding_idx = 0.0
                    if key in dp.batch and dp.batch[key] is not None:
                        current_len = dp.batch[key].shape[1]
                        pad_size = max_input_len - current_len
                        if pad_size > 0:
                            pad_shape = (dp.batch[key].shape[0], pad_size) + dp.batch[key].shape[2:]
                            padding = torch.full(pad_shape, padding_idx, dtype=dp.batch[key].dtype, device=dp.batch[key].device)
                            dp.batch[key] = torch.cat([dp.batch[key], padding], dim=1)
                
                # 对响应和价值相关keys使用max_response_len

                for key in response_key_list + other_key_list:
                    if key in token_padding_list:
                        padding_idx = id_pad
                    else:
                        padding_idx = 0.0
                    if key in dp.batch and dp.batch[key] is not None:
                        current_len = dp.batch[key].shape[1]
                        pad_size = max_response_len - current_len
                        if pad_size > 0:
                            pad_shape = (dp.batch[key].shape[0], pad_size) + dp.batch[key].shape[2:]
                            padding = torch.full(pad_shape, padding_idx, dtype=dp.batch[key].dtype, device=dp.batch[key].device)
                            dp.batch[key] = torch.cat([dp.batch[key], padding], dim=1)
            for dp in data:
                if 'uid' not in dp.non_tensor_batch:
                    dp.non_tensor_batch['uid'] = dp.non_tensor_batch['index'].copy()
            batch_lst = []
            for batch in data:
            ...
            for key, val in non_tensor_batch.items():
                # print(key, [v.shape for v in val])
                try:
                    non_tensor_batch[key] = np.concatenate(val, axis=0)
                except:
                    raise ValueError(key, [v.shape for v in val])
        else:
            batch_lst = []
            for batch in data:
                batch_lst.append(batch.batch)
