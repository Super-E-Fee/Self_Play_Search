...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        self.data_pool_manager = Data_pool_manager(
            solver_pool_size=self.config.data.train_batch_size, 
            update_ratio=0.1, neg_pro_ratio=0.5, 
            pad_token_id=self.tokenizer.pad_token_id, 
            divisor=self.config.trainer.n_gpus_per_node * self.config.trainer.nnodes)
    ...
    def fit(self):
        ...
        with tqdm(total=self.total_training_steps) as pbar:
            ...
            for epoch in range(self.config.trainer.total_epochs):
                ...
                self.data_pool_manager.clear_at_step_start()

                self.data_pool_manager.pool_add_data(max_retry_times=1)################################################# 实现这里 ##########################
...
class Data_pool:
    ...
    def pop_data(self, pop_num:int=1):
        if self.pool is None:
            raise ValueError("Proposer pool is empty.")
        if pop_num > len(self.pool.non_tensor_batch['id']):
            pop_num = len(self.pool.non_tensor_batch['id'])
        
        pop_indices = random.sample(range(len(self.pool.non_tensor_batch['id'])), pop_num)
        pop_indices_tensor = torch.tensor(pop_indices, dtype=torch.long)

        pop_data = self.pool[pop_indices_tensor]

        # 更新pool，移除已弹出的数据
        mask = torch.ones(len(self.pool.batch), dtype=torch.bool)
        mask[pop_indices] = False

        non_tensor_batch_keys = list(self.pool.non_tensor_batch.keys())
        self.pool.batch = self.pool.batch[mask]
        for key in non_tensor_batch_keys:
            self.pool.non_tensor_batch[key] = copy.deepcopy(self.pool.non_tensor_batch[key][mask.numpy()])
        return pop_data        
...
class Data_pool_manager:
    def __init__(self, solver_pool_size=1024, update_ratio=0.1, neg_pro_ratio=0.5, pad_token_id=None, divisor: int=2):
        
        self.solver_qa_pool = Data_pool(pad_token_id=pad_token_id, divisor=divisor) # 存储生成完毕的用于solver的QA对

        self.proposer_pool = Data_pool(pad_token_id=pad_token_id, divisor=divisor) # 存储从seed 生成的 positive response，将会用于生成qa negative response，只会用于 proposer 训练

        self.pre_qa_pool = Data_pool(pad_token_id=pad_token_id, divisor=divisor) # 存储从seed 生成的 positive response，将只会用于生成qa
        
        self.solver_qa_pool_size = num_div_ver(solver_pool_size) # 整除 预计的并行分节点 num

        proposer_pos_size = num_div_ver(int(update_ratio*solver_pool_size*(1 - neg_pro_ratio))) # 整除 预计的并行分节点 num

        self.proposer_pool_size = proposer_pos_size

        proposer_nega_size = num_div_ver(int(neg_pro_ratio*solver_pool_size*neg_pro_ratio)) # 整除 预计的并行分节点 num

        self.proposer_pool_nega_size = proposer_nega_size

        self.pad_token_id = pad_token_id[0]

        self.divisor = divisor

        def num_div_ver(input_num:int):
            left_part = int(input_num%self.divisor)
            if left_part ==0:
                return input_num
            else:
                return int(input_num + (self.divisor - left_part))
            

    def clear_at_step_start(self):
        
        pop_num_aim = self.proposer_pool.get_len() - (self.solver_qa_pool_size - self.proposer_pool_size)

        if pop_num_aim >= 0:
            # pop 掉被更新的问题
            self.solver_qa_pool.pop_data(pop_num=pop_num_aim)
        
        # 清空 proposer 侧的数据池
        self.proposer_pool.clear_all()
        self.pre_qa_pool.clear_all()

        gc.collect()

    def pool_add_data(self, max_retry_times=3):
        ...
        demanded_pos_qa_num = self.solver_qa_pool_size - self.solver_qa_pool.get_len()

        pos_qa_pool_size = 0

        while pos_qa_pool_size<demanded_pos_qa_num: # self.proposer_pool.get_len() < self.solver_qa_pool_size or self.solver_qa_pool.get_len() < self.proposer_pool_size

            pre_pos_qa_len = len(pos_qa_pool.non_tensor_batch['id']) if pos_qa_pool is not None else 0
            # pre_nega_qa_len = len(nega_qa_pool.non_tensor_batch['id']) if nega_qa_pool is not None else 0

            qa_dp = generate_qa()

            pos_qa, nega_qa = evaluate_qa(qa_dp)


            if pos_qa is not None:

                if pos_qa_pool is not None:
                    pos_qa_pool = DataProto.concat([pos_qa_pool, pos_qa], auto_pad=True, key_list=list(pos_qa_pool.batch.keys()), id_pad=self.pad_token_id)
                else:
                    pos_qa_pool = pos_qa


            if nega_qa is not None:
                if nega_qa_pool is not None:
                    nega_qa_pool = DataProto.concat([nega_qa_pool, nega_qa], auto_pad=True, key_list=list(nega_qa_pool.batch.keys()), id_pad=self.pad_token_id)
                else:
                    nega_qa_pool = nega_qa

            pos_qa_pool_size = len(pos_qa_pool.non_tensor_batch['id']) if pos_qa_pool is not None else 0




            if pos_qa_pool_size - pre_pos_qa_len ==0:
                retry_times -=1
            else:
                retry_times = max_retry_times
            if max_retry_times <=0:
                raise ValueError(f"Data pool manager generate qa failed more than {max_retry_times} times.")


        
        if self.solver_qa_pool.get_len()>0:

            solver_existing_ids_max = self.solver_qa_pool.get_len()

            self.solver_qa_pool.non_tensor_batch['id'] = list(range(solver_existing_ids_max))
        else:
            solver_existing_ids_max =0

        pos_qa_pool_size = len(pos_qa_pool.non_tensor_batch['id']) if pos_qa_pool is not None else 0
        pos_max = solver_existing_ids_max+pos_qa_pool_size
        pos_qa_ids = list(range(solver_existing_ids_max,pos_max))
        pos_qa_pool.non_tensor_batch['id'] = pos_qa_ids


        nega_qa_pool_size = len(nega_qa_pool.non_tensor_batch['id']) if nega_qa_pool is not None else 0
        if nega_qa_pool_size> 0:
            nega_qa_max = pos_max+nega_qa_pool_size
            nega_qa_ids = list(range(pos_max,nega_qa_max))
            nega_qa_pool.non_tensor_batch['id'] = nega_qa_ids
        


        train_qa_use = pos_qa_pool[:demanded_pos_qa_num]       

        pos_qa_use = pos_qa_pool[:self.proposer_pool_size]


        self.solver_qa_pool.add(train_qa_use)
        self.proposer_pool.add(pos_qa_use)


        if nega_qa_pool_size >= self.proposer_pool_nega_size:
            nega_qa_use = nega_qa_pool[:self.proposer_pool_nega_size]
            self.proposer_pool.add(nega_qa_use)

    
    def from_pre_to_qa_pool(self):
        pass

    def generate_qa(self):
        pass

    def evaluate_qa(self, qa_dp:DataProto):
        pass
