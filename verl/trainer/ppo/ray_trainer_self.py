...
class RayPPOTrainer(object):
    ...
    def init_workers(self):
        ...
        self.proposer_rollout_wg = all_wg['ref']
    ...
    def fit(self):
        ...
        proposer_warmup = 10 + self.config.trainer.critic_warmup

        global_end = 0
        proposed_pool = Proposer_pool(pad_token_id=self.pad_token_id)
        # start training loop
        with tqdm(total=self.total_training_steps) as pbar:
            ...
            for epoch in range(self.config.trainer.total_epochs):
                ...
                # proposed_pool.clear_keys_in_new_step()
                ...
                if len(q_ne_l) > 0:
                    ...
                    # proposed_pool.update_pool(idx_ne_l)
                
                # if not (len(q_ne_l) == len(a_ne_l) == len(i_ne_l) == len(idx_ne_l) == proposed_pool.get_len()):
                #     raise ValueError(f"Length mismatch q_nel:{len(q_ne_l)} a_nel:{len(a_ne_l)}  i_nel:{len(i_ne_l)}  idx_nel:{len(idx_ne_l)} pool_size:{proposed_pool.get_len()}.")
                ...
                if len(idx_ab_l) >= negative_sample_num:                    
                    ...
                    if len(idx_ab_l) == 0 and positive_sample_num%2!=0:
                        idx_po_l = idx_po_l[:len(idx_po_l)-1]
                    elif (len(idx_ab_l) + positive_sample_num)%2 != 0:
                        idx_ab_l = idx_ab_l[:len(idx_ab_l)-1]
                ...
                seed_size = seed_batch.batch['attention_mask'].shape[0]

                output_text = '*'*20 + '\n' + f'step: {self.global_steps}. \n' + f'seed batch size: {seed_size}\n'
                for i in range(len(solver_batch_list)):
                    solver_size = solver_batch_list[i].batch['attention_mask'].shape[0]
                    output_text += f'solverbatch_i: {i}. \n' + f'seed batch size: {solver_size}\n'
                ...
                for batch in solver_batch_list:
                    batch.pop(batch_keys=['p_attention_mask', 'p_position_ids', 'n_attention_mask', 'n_position_ids', 'n_input_ids', 'p_input_ids'])
    ...
    def proposer_reward_fn(self, seed_batch: DataProto, solver_batch_list: DataProto, is_debug=False, alpha=0.1, format_score=0.1):
        # 使用 with torch.no_grad() 避免计算图保留
        with torch.no_grad():
            ...
            for i in range(len(seed_batch)):
                ...
                if is_debug:
                    sum_value += score
                proposer_reward_tensor[i, valid_response_length - 1] = score
...
class TrainData(Dataset):
    ...
    def __getitem__(self, item):
        ...
        position_ids = compute_position_id_with_mask(attention_mask)
        p_position_ids = compute_position_id_with_mask(attention_mask)
        n_position_ids = compute_position_id_with_mask(attention_mask)
...
class Proposer_pool:
    ...
    def clear_keys_in_new_step(self):
        keys = ['token_level_scores', 'token_level_rewards', 'advantages', 'returns']
