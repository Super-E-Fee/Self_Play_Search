...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        self.role = role
        assert self.role in ['actor', 'rollout', 'ref', 'actor_rollout', 'actor_rollout_ref','ext']

        self._is_actor = self.role in ['actor', 'actor_rollout', 'actor_rollout_ref']
        self._is_rollout = self.role in ['rollout', 'actor_rollout', 'actor_rollout_ref']
        self._is_ref = self.role in ['ref', 'actor_rollout_ref']
        self._is_ext = self.role in ['ext']
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
            from verl.workers.rollout import HFRollout_self as HFRollout
            from verl.workers.sharding_manager import BaseShardingManager
            rollout = HFRollout(module=self.actor_module_fsdp, config=self.config.rollout, extend_ratio=extend_ratio)
            rollout_sharding_manager = BaseShardingManager()
            # TODO: a sharding manager that do nothing?
        elif self.config.rollout.name == 'vllm':
            from verl.workers.rollout.vllm_rollout import vLLMRollout_self as vLLMRollout
    ...
    def init_model(self):
        from verl.workers.actor import DataParallelPPOActor_self
        ...
        if self._is_actor or self._is_rollout or self._is_ext:
        ...
        if self._is_actor:
            ...
            self.actor = DataParallelPPOActor_self(config=self.config.actor,
        ...
        if self._is_ref:
            ...
            self.ref_policy = DataParallelPPOActor_self(config=self.config.ref, actor_module=self.ref_module_fsdp)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_sequences_ext(self, prompts: DataProto):
        # torch.cuda.empty_cache()
        prompts = prompts.to('cuda')
        input_ids_key = prompts.meta_info.get('input_ids_key', 'p_input_ids')
        attention_mask_key = prompts.meta_info.get('attention_mask_key', 'p_attention_mask')
        position_ids_key = prompts.meta_info.get('position_ids_key', 'p_position_ids')
        responses_key = prompts.meta_info.get('responses_key', 'p_responses')
        prompt_key = prompts.meta_info.get('prompt_key', 'p_prompt')
        ...
        with self.rollout_sharding_manager:
            ...
            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            output = self.rollout.generate_sequences_ext(prompts=prompts,
